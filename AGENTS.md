...
1. API (`/api/*`)
2. Uploads (`/uploads/*`) — `express.static` from `backend/uploads/`
3. Frontend static assets — `express.static` from `frontend/dist/`
4. Catch-all `*` — serves `frontend/dist/index.html` for SPA client-side routing
...
- Frontend dev server proxies `/api` and `/uploads` to `http://localhost:5000` (see `frontend/vite.config.js`)
